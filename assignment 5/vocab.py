...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        word_ids = [[[self.start_of_word]+[self.char2id[ch] for ch in word]+[self.end_of_word] 
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        words_padded = pad_sents_char(word_ids, self.char2id['<pad>'])
        sents_var = torch.tensor(words_padded, dtype=torch.int, device=device)
