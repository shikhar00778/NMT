...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(2*hidden_size, hidden_size, bias=True)
        self.h_projection = nn.Linear(2*hidden_size, hidden_size, bias=False)
        self.c_projection = nn.Linear(2*hidden_size, hidden_size, bias=False)
        self.att_projection = nn.Linear(2*hidden_size, hidden_size, bias=False)
        self.combined_output_projection = nn.Linear(3*hidden_size, hidden_size, bias=False)
        self.target_vocab_projection = nn.Linear(2*hidden_size, hidden_size, bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        init_decoder_hidden = self.h_projection(last_hidden.permute(1,0,2).contiguous().view(orig_len, -1))
        init_decoder_cell = self.c_projection(last_cell.permute(1,0,2).contiguous().view(orig_len, -1))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        soft = nn.Softmax(1)
        alpha_t = soft(e_t).unsqueeze(1)
        a_t = torch.bmm(alpha_t, enc_hiddens).squeeze()
